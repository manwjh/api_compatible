...
L2 探测：`./experiment/user-side/scripts/probe-endpoints.sh <site>`（见 [中转站调研 §2.1](./中转站主流技术栈调研.md#21-本仓库-l2-探测方法)）。
...
维护者在 `experiment/user-side/sites.json` 增加 `newapi-prototype`（host 按实际修改），`experiment/user-side/.env` 配置 `NEWAPI_PROTOTYPE_TOKEN`。模板见 [EC2-中转站原型 §10](../experiment/EC2-中转站原型实验点设计.md#10-交付用户侧sitesjson)。

### 13.3 评估流程

1. 运营商侧：Channel 通、发 Token  
2. 用户侧：`cd experiment/user-side && ./scripts/run-user-side-compat.sh --site newapi-prototype` 或 `probe-endpoints` + `./t_* -y`  
...
| 脚本 | 用途 |
|------|------|
| `experiment/user-side/scripts/probe-endpoints.sh` | L2 四端点探测 |
| `experiment/user-side/scripts/run-user-side-compat.sh` | 用户侧 Runner 一键 probe + 可选 smoke |
| `experiment/user-side/scripts/poc-litellm-bai-codex.sh` | LiteLLM POC（非 New API） |
...
**摘要**：New API 是 **Go/Gin + GORM** 的多租户 LLM 网关，以 **`RelayFormat` + `relay/channel` Adaptor** 统一 Chat、Messages、Responses、Gemini、Realtime 等入站形态，以 **Channel + Token + 分组倍率** 完成路由与计费。读源码时抓住 **`router/relay-router.go` → `controller.Relay` → `middleware.Distribute` → `GetAdaptor`** 四条线；接入本仓库时先在 **`experiment/user-side/`** 下用 **`probe-endpoints`** 确认三主端点，再跑 **`t_*`** 写报告。
