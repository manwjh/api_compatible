...
```bash
./experiment/user-side/scripts/probe-endpoints.sh <site>   # 默认 experiment/user-side/sites.json → default_site
```

实现位于 `experiment/user-side/lib/maas.py` → `cmd_probe`：对站点 `base_url` 依次探测四项，并输出 **Agent readiness（protocol probe）**。
...
`experiment/user-side/sites.json` 注册约定：
...
2. ./experiment/user-side/scripts/probe-endpoints.sh <site>  → L2
3. 缺端点？
