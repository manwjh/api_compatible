...
| **调研基线** | New API `v1.0.0-rc.9`（2026-05-26）· One API `main` 活跃分支 · LiteLLM Docker `-stable` 线 · b.ai 本仓库 E3（2026-06-01） |
| **复审触发** | 主流网关大版本变更、新增或废弃 `/v1/responses` / Realtime 等端点、本仓库新增站点 E3 报告 |
| **待补实测** | 自托管 New API × 三 Agent 的 L3–L5（当前仅 b.ai 有完整 E3）；境外原型见 [EC2-中转站原型实验点设计.md](../experiment/EC2-中转站原型实验点设计.md)，用户侧 Runner 见 [EC2-用户侧隔离实验点设计.md](../experiment/EC2-用户侧隔离实验点设计.md) |
...
与 **原厂 API** 的差异：中转站 **不拥有模型算力**，价值在于 **聚合、定价与可用性**；对外协议面 **可能不完整**（参见 [Codex × b.ai 报告](../reports/Codex兼容性评估报告.md)）。
...
| 商业 Token 站（**b.ai** 为本仓库唯一完整 E3 样例） | 各站定价、合规条款、服务等级协议 |
...
| **E3** | **本仓库** `probe-endpoints` + `reports/` 实测 | b.ai × 三 Agent（2026-06-01） |
| **E4** | **本仓库** 自托管网关 × Agent 的 L3–L5 | **尚无** — 待补 |
...
```bash
./scripts/probe-endpoints.sh <site>   # 默认 sites.json → default_site
```

实现位于 `lib/maas.py` → `cmd_probe`：对站点 `base_url` 依次探测四项，并输出 **Agent readiness（protocol probe）**。

| 探测 | HTTP 方法 | 路径 | 映射 Agent |
|------|-----------|------|------------|
| 模型列表 | GET | `/v1/models` | 辅助（模型发现） |
...
| 结果 | 常见含义 | 处理建议 |
|------|----------|----------|
| **200** | 端点存在且 Key/模型组合可用 | 继续 L3–L5（`./t_*`） |
...
**b.ai（E3）**：`/v1/responses` 返回 **403** 且 body 明确列出允许路径 — 属于 **策略性裁剪**，而非临时故障：
...
| **商业 Token 站** | b.ai 等 | 闭源 SaaS | 按量购 Token 的开发者 | E3（b.ai） |
...
本仓库可通过 `./scripts/pull-upstream.sh newapi` 拉取 New API 参考源码（`.gitignore`，不纳入版本库）。
...
### 7.2 商业 Token 站：b.ai（本仓库 E3）

当前 **唯一** 具备三 Agent 同环境对比报告的商业站。

| 探测项 | 结果 | 证据 |
|--------|------|------|
| `GET /v1/models` | 通过 | E3 |
| `POST /v1/chat/completions` | 通过 | E3 → OpenCode 兼容 |
| `POST /v1/messages` | 通过（部分模型受账户权限限制） | E3 → Claude Code 基本兼容 |
| `POST /v1/responses` | **拒绝**（403，路径白名单） | E3 → Codex 不兼容 |

`sites.json` 注册约定：

| 字段 | b.ai 取值 | 用途 |
|------|-----------|------|
| `base_url` | `https://api.b.ai/v1` | OpenCode / Codex probe、`openai_base_url` |
| `anthropic_base_url` | `https://api.b.ai` | Claude Code（Messages 位于 `/v1/messages`） |

商业站 **实现栈通常不公开**。运维上可能基于 New API fork 或完全自研 — **协议面仅以 probe 与 reports 为准**。
...
| **`probe-endpoints` 四项结果** | **唯一** 可纳入本仓库结论的 L2 依据 |
...
2. ./scripts/probe-endpoints.sh <site>  → L2
3. 缺端点？
   ├─ 换站 / 自托管启用 RelayFormat
   └─ 或叠加 Agent 侧桥接（见协议转换调研）
4. ./t_* --site <site> -y  → L3–L5
5. 写入 docs/reports/
```
...
```json
{
  "example-site": {
    ...
    "notes": "L2: Chat + Messages OK; Responses unknown — run probe-endpoints"
  }
}
```
...
| 步骤 | 动作 |
|------|------|
| 1 | 编辑 `sites.json`、`.env.example` |
| 2 | `./scripts/probe-endpoints.sh <site>` |
| 3 | `./t_claude` / `./t_codex` / `./t_opencode`（建议 `-y` 非交互探针） |
| 4 | 新增 `docs/reports/<站点>-<Agent>兼容性评估报告.md` |
| 5 | 可选：`./scripts/pull-upstream.sh newapi` 对照 Relay 实现 |
...
| 局限 | 说明 |
|------|------|
| **E3 覆盖不足** | 仅 b.ai 有完整三 Agent 报告；自托管 New API / LiteLLM 的 L3–L5 未在本仓库验证 |
...
| 403 误判为「无端点」 | 选型错误 | 读 response body（如 b.ai 白名单提示） |
| 模型 ID / 权限 | 404 或 403 | `./t_* --list-models` + 账户权限 |
...
- [b.ai 三 Agent 对比](../reports/README.md#ba2026-06-01)

---

**摘要**：中转站实现栈可归纳为 **Go/Gin 系（One API → New API）** 与 **Python/FastAPI 系（LiteLLM）** 两条主线；商业 Token 站实现不透明，**协议面不可推测**。接入 Coding Agent 时，先以 **`probe-endpoints` 完成 L2**，再以 **`t_*` 与 reports 完成 L3–L5**；产品文档（E1）与站点实测（E3）须分开引用。
